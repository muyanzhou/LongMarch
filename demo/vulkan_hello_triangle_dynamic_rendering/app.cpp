...
void Application::OnInit() {
  ...
  long_march::vulkan::PipelineSettings pipeline_settings(
      pipeline_layout_.get(), {VK_FORMAT_B8G8R8A8_UNORM});
...
void Application::OnRender() {
  ...
  VkExtent2D extent = frame_image_->Extent();
  VkViewport viewport{};
  viewport.x = 0.0f;
  viewport.y = 0.0f;
  ...
  copy_region.extent.width = frame_image_->Extent().width;
  copy_region.extent.height = frame_image_->Extent().height;
