...
void grassland::vulkan::Core::RegisterGLFWWindowEventCallbacks() {
    if (settings_.window) {
        ...
        glfwSetCursorPosCallback(settings_.window, [](GLFWwindow *window, double xpos,
                                                      double ypos) {
            auto core = reinterpret_cast<Core *>(glfwGetWindowUserPointer(window));
            core->cursor_pos_event_.InvokeCallbacks(xpos, ypos);
        });

        glfwSetMouseButtonCallback(settings_.window,
        ...
        glfwSetDropCallback(settings_.window,
                            [](GLFWwindow *window, int path_count,
                               const char **paths) {
                                auto core = reinterpret_cast<Core *>(glfwGetWindowUserPointer(window));
                                core->drop_event_.InvokeCallbacks(path_count, paths);
                            });

        glfwSetScrollCallback(settings_.window,
